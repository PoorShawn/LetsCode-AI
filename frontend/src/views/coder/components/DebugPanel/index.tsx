    ...
    <div className="flex flex-col bg-[#1e1e1e] text-gray-100 rounded overflow-hidden w-full border border-[#333333] mb-2.5">
      ...
      {isDebugMode && (
        <div className="flex gap-1.5 p-2 bg-[#252525] border-b border-[#444444] overflow-x-auto">
          ...
          <button className="debug-button" title="单步执行" disabled={true}>
            单步
          </button>
          <button className="debug-button" title="步入函数" disabled={true}>
            步入
          </button>
          <button className="debug-button" title="步出函数" disabled={true}>
            步出
          </button>
          <button className="debug-button" title="继续执行" disabled={true}>
            继续
          </button>
          <button className="debug-button" title="停止调试" disabled={true}>
            停止
          </button>
        </div>
      )}
